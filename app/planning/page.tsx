...
export default function PlanningPage() {
  const { planningTasks, loadFromStorage, addTask, updateTask, deleteTask, toggleTaskDone, toggleChecklistItem } = useTaskStore();
  ...
  const handleAdd = (task: Omit<Task, "id" | "done">) => {
    addTask("planning", task);
  };

  const handleEdit = (task: Omit<Task, "id" | "done">) => {
    if (editingTask) {
      updateTask("planning", editingTask.id, task);
    }
  };

  const handleDelete = () => {
    if (editingTask) {
      deleteTask("planning", editingTask.id);
      setModalOpen(false);
      setEditingTask(null);
    }
  };
    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="flex flex-col gap-3">
          ...
          <Card key={task.id} className="hover:bg-gray-900/50 transition-colors cursor-pointer">
            <div onClick={() => openEditModal(task)}>
              <div className="flex items-start gap-3">
                <div onClick={(e) => { e.stopPropagation(); toggleTaskDone("planning", task.id); }}>
        ...
        {planningTasks.length === 0 && (
          <p className="text-gray-500 text-sm text-center py-8">No tasks yet. Click "+ Add New" to create one.</p>
        )}
      </div>
